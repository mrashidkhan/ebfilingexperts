<!-- What Makes EB Filing Experts Different Section -->
<section class="makes-eb1a-experts-different-main ptb-60 background-service">
    <div class="container p-0">
        <!-- Section Title -->
        <div class="row pb-4">
            <div class="col-lg-10 col-md-10 col-sm-12 col-xs-12 m-auto text-center">
                <div class="makes-eb1a-experts-different">
                    <h2>
                        What sets EB Filing Experts apart from other services?
                    </h2>
                    <p>
                        EB Filing Experts is trusted by tech professionals for seamless, strategic EB-1A success.
                    </p>
                </div>
            </div>
        </div>

        <!-- Features Grid -->
        <div class="features-area item-full text-center cell-items default-padding">
            <div class="row features-items">
                <!-- Feature 1: AI-Driven Profile Optimization -->
                <div class="col-md-4 col-sm-6 equal-height">
                    <div class="item">
                        <div class="icon">
                            <div class="thumbnail">
                                <img src="{{ asset('assets/images/inner-images/why-choose-us-icon1.png') }}" alt="AI-Driven Profile Optimization">
                            </div>
                        </div>
                        <div class="info">
                            <h4>

                                    AI-Powered Profile Optimization
                                
                            </h4>
                            <p>
                                AI Athena optimizes every detail of your profile to maximize your EB-1A approval potential.
                            </p>
                        </div>
                    </div>
                </div>

                <!-- Feature 2: Data-Backed Qualification Insights -->
                <div class="col-md-4 col-sm-6 equal-height">
                    <div class="item">
                        <div class="icon">
                            <div class="thumbnail">
                                <img src="{{ asset('assets/images/inner-images/why-choose-us-icon2.png') }}" alt="Data-Backed Qualification Insights">
                            </div>
                        </div>
                        <div class="info">
                            <h4>Data-Driven Eligibility Insights</h4>
                            <p>
                                Our AI compares your credentials to successful EB-1A cases, creating a clear strategy to strengthen your application.
                            </p>
                        </div>
                    </div>
                </div>

                <!-- Feature 3: Expert-Led Strategy & Personalization -->
                <div class="col-md-4 col-sm-6 equal-height">
                    <div class="item">
                        <div class="icon">
                            <div class="thumbnail">
                                <img src="{{ asset('assets/images/inner-images/why-choose-us-icon3.png') }}" alt="Expert-Led Strategy">
                            </div>
                        </div>
                        <div class="info">
                            <h4>Expert Strategy, Personalized for You</h4>
                            <p>
                                Our EB-1A strategists tailor your profile to meet USCIS expectations and industry standards.
                            </p>
                        </div>
                    </div>
                </div>

                <!-- Feature 4: Dedicated Case Manager -->
                <div class="col-md-4 col-sm-6 equal-height">
                    <div class="item">
                        <div class="icon">
                            <div class="thumbnail">
                                <img src="{{ asset('assets/images/inner-images/why-choose-us-icon4.png') }}" alt="Dedicated Case Manager">
                            </div>
                        </div>
                        <div class="info">
                            <h4>Dedicated EB-1A Case Expert</h4>
                            <p>
                                EEvery client gets a dedicated case manager and 24/7 support for a smooth experience.
                            </p>
                        </div>
                    </div>
                </div>

                <!-- Feature 5: High-Impact Recommendation Letters -->
                <div class="col-md-4 col-sm-6 equal-height">
                    <div class="item">
                        <div class="icon">
                            <div class="thumbnail">
                                <img src="{{ asset('assets/images/inner-images/why-choose-us-icon5.png') }}" alt="High-Impact Recommendation Letters">
                            </div>
                        </div>
                        <div class="info">
                            <h4>
                                High-Impact
                                <a href="{{ route('case-studies') }}" target="_blank" class="target-color">
                                    Powerful Recommendation Letters
                                </a>
                            </h4>
                            <p>
                               Our experts craft strategic recommendation letters highlighting your achievements and industry leadership.
                            </p>
                        </div>
                    </div>
                </div>

                <!-- Feature 6: Intelligent Gap-Filing Framework -->
                <div class="col-md-4 col-sm-6 equal-height">
                    <div class="item">
                        <div class="icon">
                            <div class="thumbnail">
                                <img src="{{ asset('assets/images/inner-images/why-choose-us-icon6.png') }}" alt="Intelligent Gap-Filing Framework">
                            </div>
                        </div>
                        <div class="info">
                            <h4>Intelligent Gap-Filing Framework</h4>
                            <p>
                                We don't just assess—we strengthen your profile. Our AI identifies gaps and strategically suggests additions that turn near-misses into winning qualifications.

                            </p>
                        </div>
                    </div>
                </div>

                <!-- Feature 7: Efficient Process & Time Optimization -->
                <div class="col-md-4 col-sm-6 equal-height">
                    <div class="item">
                        <div class="icon">
                            <div class="thumbnail">
                                <img src="{{ asset('assets/images/inner-images/why-choose-us-icon7.png') }}" alt="Efficient Process">
                            </div>
                        </div>
                        <div class="info">
                            <h4></h4>
                            <p>
                                Expert guidance that speeds up your EB-1A journey while maintaining quality.
                            </p>
                        </div>
                    </div>
                </div>

                <!-- Feature 8: A Proven Track Record of Success -->
                <div class="col-md-4 col-sm-6 equal-height">
                    <div class="item">
                        <div class="icon">
                            <div class="thumbnail">
                                <img src="{{ asset('assets/images/inner-images/why-choose-us-icon8.png') }}" alt="Proven Track Record">
                            </div>
                        </div>
                        <div class="info">
                            <h4>Success Stories. Proven Results.</h4>
                            <p>
                                We consistently win EB-1A approvals for leading tech professionals through strategic, precise execution.
                            </p>
                        </div>
                    </div>
                </div>

                <!-- Feature 9: Comprehensive Criterion Coverage -->
                <div class="col-md-4 col-sm-6 equal-height">
                    <div class="item">
                        <div class="icon">
                            <div class="thumbnail">
                                <img src="{{ asset('assets/images/inner-images/why-choose-us-icon9.png') }}" alt="Comprehensive Criterion Coverage">
                            </div>
                        </div>
                        <div class="info">
                            <h4>Full Coverage of All EB-1A Criteria</h4>
                            <p>
                                Deep Strategic Positioning of Publications, Judging, Memberships, and More
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
